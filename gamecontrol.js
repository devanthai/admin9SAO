...
router.get('/rollback', async (req, res) => {
    ...
    for (let i = 0; i < clans.length; i++) {
        ...
        for (let iz = 0; iz < checkusClan.length; iz++) {
            ...
            var thanhtichuser = item.thanhtichngay
            const cc = await Clan.updateOne({ _id: item.clan.id }, { $inc: { thanhtich: thanhtichuser } })
        }
...
async function newDayTraogiao() {
    try {
        ...
        try {
            BotDonRac.sendMessage(-728949790, "đã trao giải bang hội tổng: " + sovanggggg)
    ...
    try {
        ...
        for (let i = 0; i < topUser.length; i++) {
            ...
            else if (indexTop == 7) { giaithuong = 10000000 }

            giaithuong += topUser[i].thanhtichngay * (indexTop < 8 ? 0.05 : 0.02)
        ...
        try {
            BotDonRac.sendMessage(-728949790, "đã trao giải cá nhân: " + sovanggggggggg)
        }
        catch { }
    }
    catch { }
